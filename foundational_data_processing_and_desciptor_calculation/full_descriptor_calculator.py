'''
A code that will take a list of refcodes from a gcd file and return individual csv files of each crystal with all it's descriptor information included

CAUTION: The file must have the refcodes in a list going downwards (as shown below) otherwise this code will not work.
...
Made by Leo Arogundade (10/07/2024)
'''
...
def refcode_list_to_all_descriptor_csv_files(gcd_file_path, csv_newfile_folder_path):
    '''
    A function that calculates all descriptors and stores them in .csv files
    
    Input: The path file for the .gcd file with the refcodes
    Output: A .csv for each refcode which has the values of the 2d descriptors
    '''
    
    refcodelist = gcdfile_to_refcode_list(gcd_file_path) # Obtain the refcode list
    failed_molecule_refcodes_list = [] # Create a list to store any failed refcodes
    ...
    for refcode in refcodelist: # Loop through the refcodes
        ...
        if mol_rdkit is None: # Check if the molecule generation was successful
            print(f'Failed to load molecule from file')  # Gives error message
            failed_molecule_refcodes_list.append(refcode) # Stores refcode in failed list
        else: 
            csd_reader = io.EntryReader('CSD') # Connecting to the CSD database
            mol_ccdc = csd_reader.molecule(refcode) # Getting the molecule from CSD database

            # Define the descriptors that we want to store
            descriptors = [
            ...
            csv_newfile_path = rf'{csv_newfile_folder_path}\3d_descriptors_{refcode}.csv' # Define the path for the new CSV file
            with open(csv_newfile_path, 'w', newline='') as csvfile: # Open the file in write mode
                writer = csv.writer(csvfile) # Create a CSV writer object
                for descriptor in descriptors: # Iterate over each descriptor
                    writer.writerow(descriptor) # Write the descriptor to the CSV file
                print(f"Descriptors written to {csv_newfile_path} successfully.") # Confirm successful writing
...
# GCD file path
gcd_file_path = r'racemic_refcodes.gcd'
# Path to folder where the .csv files will be stored
csv_newfile_folder_path = r'racemic'
refcode_list_to_all_descriptor_csv_files(gcd_file_path,csv_newfile_folder_path)
